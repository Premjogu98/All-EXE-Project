...
def Scraping_function(Source):
    ...
    while a < 3:
        try:
            ...
            request = urllib.request.Request('http://agricoop.gov.in/tenders?field_tender_type_tid=All', None, headers)  # The assembled request
            ...
            for td in tr_list:
                ...
                for data in range(42):
                    SegField.append('')
                SegField[1] = 'secy-agri@nic.in'
                SegField[2] = 'Ministry of Agriculture and Farmers Welfare, Krishi Bhawan, Rajendra Prasad Road, New Delhi-110001<br>\nTel: 0413-2336945, 2336543, 2336061<br>\nFax: 0413-2337121'
                SegField[3] = "NA~NA~NA~NA~NA"
                SegField[8] = 'http://agricoop.gov.in'  # Enter Here source website Its IMP
                # =======================================================================================
                ...
                SegField[28] = 'http://agricoop.gov.in/tenders?field_tender_type_tid=All'
                SegField[31] = "agricoop.gov.in"
                ...
                clean = re.compile('<.*?>')
                Title = re.sub(clean, '', str(td_list[1]))
                Title = string.capwords(str(Title)).strip()
                SegField[19] = Title
                # print(Title)

                SegField[12] = 'AGRICULTURE, COOPERATION & FARMERS WELFARE'

                Document = str(td_list[2]).partition("href=\"")[2].partition("\"")[0].strip()
                # print(Document)
                SegField[5] = Document

                clean = re.compile('<.*?>')
                Closing_Date = re.sub(clean, '', str(td_list[3])).strip()
                # print(Closing_Date)
                if Closing_Date != '':
...
def Create_HTML_string(SegField,Source):
    try:
        ...
        globalvar.Col1 = 'http://agricoop.gov.in'
